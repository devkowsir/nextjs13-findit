...
const PostMetadata: React.FC<PostMetadataProps> = ({
  ...
  const topicMeta = (
    <div className="relative rounded bg-slate-50 px-2 py-1" ref={topicRef}>
      <Link
      ...
      {isTopicInfoVisible &&
        (topicInfo ? (
          <div className="absolute left-0 top-full z-10 w-80 space-y-2 rounded-md bg-slate-50 px-3 py-2 shadow">
  ...
  const authorMeta = (
    <div ref={userRef} className="relative rounded bg-slate-50 px-2 py-1">
      <span className={`${topicName ? "hidden sm:inline" : ""}`}>
        Posted by{" "}
      </span>
      <Link
        href={`/u/${username}`}
